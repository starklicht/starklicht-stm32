 ...
 */
OneButton::OneButton(GPIO_TypeDef *gpio, const uint16_t pin, const bool activeLow, const bool pullupActive)
{
  // OneButton();
  _pin = pin;
  _gpio = gpio;
...
void OneButton::tick(void)
{
  if (_pin >= 0)
  {
    tick(HAL_GPIO_ReadPin(_gpio, _pin) == _buttonPressed);
  }
}
...
void OneButton::tick(bool activeLevel)
{
  ...
  case OneButton::OCS_DOWN:
    ...
    else if ((activeLevel) && !clickFired)
    {
      if (_startClickFunc)
        _startClickFunc();
      if (_paramStartClickFunc)
        _paramStartClickFunc(_startClickFuncParam);
      clickFired = true;
    }
